...
import my.bank.utils.CustomLogger;
...
public class ConsoleApplication {
    private static final Logger logger = LoggerFactory.getLogger(ConsoleApplication.class.getName());
    ...
    public void run() {
        ...
        CustomLogger.printStartEnd("Welcome to Banking Management System\n");
        while (isRunning) {
    ...
    private boolean showNoLoginMenu() {
        CustomLogger.printMainMenu("1. Sign Up for a New Account");
        CustomLogger.printMainMenu("2. Log In to Your Account");
        CustomLogger.printMainMenu("3. Exit the System");
        try {
            ...
            switch (choice) {
                ...
                default -> CustomLogger.warning("Please select an option (1-3):");
            }
        } catch (Exception e) {
            CustomLogger.error("An unexpected error occurred: " + e.getMessage() + ". Please try again.");
    ...
    private void exitSystem() {
        CustomLogger.printStartEnd("Goodbye!");
    ...
    private boolean showCustomerMenu() {
        CustomLogger.printStartEnd("Menu For Customers:\n");
        CustomLogger.printMainMenu("1. Show Balance");
        CustomLogger.printMainMenu("2. Deposit");
        CustomLogger.printMainMenu("3. Withdraw");
        CustomLogger.printMainMenu("4. Transfer");
        CustomLogger.printMainMenu("5. Favorite Account");
        CustomLogger.printMainMenu("6. Show Transactions History");
        CustomLogger.printMainMenu("7. Reset Password");
        CustomLogger.printMainMenu("8. Deactivate account");
        CustomLogger.printMainMenu("9. Logout");
        try {
            int choice = scanner.nextInt();
            scanner.nextLine();
            switch (choice) {
                ...
                case 9 -> logOut();
                default -> CustomLogger.warning("Please select an option (1-9):");
            }
        } catch (Exception e) {
            CustomLogger.error("An unexpected error occurred: " + e.getMessage() + ". Please try again.");
            logger.log(Level.SEVERE, "Critical operation failed", e.getMessage());
    ...
    private void showFavoriteAccountsMenu() {
        CustomLogger.printStartEnd("Choose one action:");
        CustomLogger.printSubMenu("1. Show Favorite Accounts");
        CustomLogger.printSubMenu("2. Add a new Favorite Account");
        CustomLogger.printSubMenu("3. Remove a Favorite Account");
        CustomLogger.printSubMenu("4. Exit Favorite Accounts Menu");
        try {
            int choice = scanner.nextInt();
            scanner.nextLine();
            switch (choice) {
                ...
                case 4 -> CustomLogger.printHint("Backing to main menu ..");
                default -> CustomLogger.warning("Choose an action between 1-4: ");
            }
        } catch (Exception e) {
            CustomLogger.error("An unexpected error occurred: " + e.getMessage() + ". Please try again.");
            logger.log(Level.SEVERE, "Critical operation failed", e.getMessage());
    ...
    private void removeFavoriteAccount() {
        ...
        CustomLogger.warning("To go back to the main menu please enter -1");
        ...
        try {
            ...
            System.out.println("Are you sure you want to remove " + CustomLogger.coloredData(userFullName) + " from your favorites? (y/n)");
            String userInput = scanner.nextLine();
            if (userInput.equalsIgnoreCase("N")) {
                CustomLogger.printInfo("Removing account from your favorite list was canceled.");
            } else if (userInput.equalsIgnoreCase("Y")) {
                accountRepository.removeFavoriteAccount(userAccount, favoriteAccount);
                CustomLogger.printInfo("Account successfully removed from your favorite list. Your changes have been saved.");
            } else {
    ...
    private void addNewFavoriteAccount() {
        ...
        try {
            ...
            Account favoriteAccount = accountRepository.getAccount(accountNumber);
            String userFullName = userRepository.getUserInfo(favoriteAccount.getUserId());
            System.out.println("Are you sure you want to add " + CustomLogger.coloredData(userFullName) + CustomLogger.resetColoredMessage(" as a favorite account? (y/n)"));
            String userInput = scanner.nextLine();
            if (userInput.equalsIgnoreCase("N")) {
                CustomLogger.printInfo("Adding account to your favorite list was canceled.");
            } else if (userInput.equalsIgnoreCase("Y")) {
                accountRepository.addNewFavoriteAccount(userAccount, favoriteAccount);
                CustomLogger.printInfo("Account successfully added to your favorite list. Thank you for using our banking system.");
            } else {
                throw new IllegalArgumentException("Invalid user's input.");
            }
        } catch (Exception e) {
    ...
    private void showAllFavoriteAccounts() {
        ...
        if (favoriteAccounts.isEmpty()) {
            CustomLogger.printInfo("You have no favorite account list yet.");
            return;
        }
        List<String> accountInfos = favoriteAccounts.stream()
                ...
                .toList();
        CustomLogger.printInfo("Your favorite accounts: ");
        CustomLogger.printInfo(String.join("\n", accountInfos));
    }
    ...
    private void ShowResetPasswordPrompt() {
        ...
        CustomLogger.warning("To go back to the main menu please enter -1");
        CustomLogger.printInfo("Your password must meet the following criteria:\n"
        ...
        CustomLogger.printInfo("You have successfully set a new password.");
    }

    private void showTransactionHistoryPrompt() {
        inputAccountPin();
        CustomLogger.printSubMenu("Choose an action:");
        CustomLogger.printSubMenu("1. Show All Transactions");
        CustomLogger.printSubMenu("2. Show Transaction History within a specified date range");
        CustomLogger.printSubMenu("3. Exit");
        ...
        try {
            ...
            switch (choice) {
                case 1 -> {
                    CustomLogger.printInfo("Your transactions history:");
                ...
                default -> CustomLogger.warning("Please select an option (1-2)");
    ...
    private void getDateAndShowTransactions() {
        ...
        CustomLogger.warning("To go back to the main menu please enter -1");
        System.out.println("Please enter the start date " + CustomLogger.coloredData("(format: dd-MM-yyyy)") + CustomLogger.resetColoredMessage(": "));
        ...
        CustomLogger.warning("To go back to the main menu please enter -1");
        System.out.println("Please enter the end date " + CustomLogger.coloredData("(format: dd-MM-yyyy)") + CustomLogger.resetColoredMessage(": "));
        ...
        CustomLogger.printInfo("Transaction history between " + fromDate + " and " + toDate);
        CustomLogger.printInfo(
                String.join("\n",
    ...
    private void showShowBalancePrompt() {
        inputAccountPin();
        CustomLogger.printInfo("Your current balance: "
                + userAccount.getBalance()
                + "USD.");
    }

    private void inputAccountPin() {
        CustomLogger.printHint("Enter your account's pin: ");
    ...
    private void showDeactivateAccountPrompt() {
        ...
        try {
            ...
            if (userInput.equalsIgnoreCase("Y")) {
                ...
                CustomLogger.printInfo("Your account was successfully deactivated! Goodbye!");
                logOut();
            } else if (userInput.equalsIgnoreCase("N")) {
                CustomLogger.printInfo("We're glad you decided to stay with us. If you need assistance, let us know!");
            } else {
    ...
    private void showLoginPrompt() {
        CustomLogger.printStartEnd("Please log into system:");
    ...
    private void showSignupPrompt() {
        ...
        CustomLogger.printInfo("You are creating a " + type + " account with the " + accountPlan + " plan. To proceed, please enter 1:");
        ...
        CustomLogger.printInfo("You have successfully created your account.");
    ...
    private int inputType() {
        ...
        do {
            try {
                ...
                CustomLogger.error("Invalid account type.");
            }
        } while (true);
        return type;
    }
    ...
    private double inputInitialDeposit(AccountPlan accountPlan) {
        ...
        do {
            try {
                CustomLogger.warning("To go back to the main menu please enter -1");
                CustomLogger.printInfo("Your deposit limit: " + accountRepository.getDepositLimit(accountPlan));
    ...
    private void showDepositPrompt() {
        inputAccountPin();
        CustomLogger.printInfo("Your account's deposit limit is: " + accountRepository.getDepositLimit(userAccount.getPlan()) + " USD.");
        System.out.println("Enter your desired deposit amount: ");
        try {
            double amount = scanner.nextDouble();
            scanner.nextLine();
            ...
            CustomLogger.printInfo("You have successfully deposited " + amount + " USD. Your new balance is: " + userAccount.getBalance() + " USD.");
    ...
    private void showWithdrawPrompt() {
        inputAccountPin();
        CustomLogger.printInfo("Your account's withdrawal limit is: " + accountRepository.getWithdrawLimit(userAccount.getPlan()) + " USD.");
        ...
        try {
            ...
            CustomLogger.printInfo("You have successfully withdrawn " + amount + " USD. Your new balance is: " + userAccount.getBalance() + " USD.");
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
